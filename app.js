...
function recordsKey() {
  ...
  return crmState.workspace === "team" ? teamRecordsKey() : `digitalWaveRecords:${userKey}`;
}

function teamRecordsKey() {
  return "digitalWaveRecords:team";
}
...
function renderTable() {
  const rows = visibleRecords().map((record) => `
    <tr data-id="${record.id}" class="${record.id === crmState.selectedId ? "selected" : ""}">
      ...
      <td><button class="badge status-button ${stageClass(record.status)}" data-action="cycle-stage" type="button">${record.status}</button></td>
      <td class="table-value" contenteditable="true" data-field="value">${record.value}</td>
...
newRecordButton.addEventListener("click", () => {
  if (crmState.view === "users") {
    moduleContent.querySelector("#teamEmailInput")?.focus();
    return;
  }
